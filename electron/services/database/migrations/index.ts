...
// Inline SQL to avoid file path issues in compiled output
...
CREATE TABLE IF NOT EXISTS app_settings (
  ...
  {
    version: 2,
    name: '002_flashcards_and_achievements',
    sql: `
-- Flashcard table: anchors the spaced repetition closed loop
...
-- Difficulty level for adaptive learning (Barrier 2)
ALTER TABLE study_sessions ADD COLUMN difficulty_level INTEGER DEFAULT 3;
    `,
  },
  {
    version: 3,
    name: '003_courses_and_goals',
    sql: `
-- Courses: maps semester/course structure to subjects
...
-- Study goals: milestone tracking for exam preparation
...
CREATE INDEX IF NOT EXISTS idx_goals_course ON study_goals(course_id);
  ...
  {
    version: 4,
    name: '004_chat_messages',
    sql: `
-- Chat messages: persist conversation history per session
...
export function runMigrations(): void {
  ...
  for (const migration of MIGRATIONS) {
    if (runVersions.has(migration.version)) {
      continue
    }
